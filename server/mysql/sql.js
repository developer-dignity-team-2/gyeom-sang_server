module.exports = {
  ...
  babsangAppliedList: `select t1.*, (select u.profile_image from user u where u.email = t1.host_email) as host_profile_image from dining_table t1 inner join dining_table_spoons t2 on t1.id = t2.dining_table_id where t2.spoon_email = ? and t2.apply_yn = 'Y'`,
  babsangCreatedList: `select t1.*, (select u.profile_image from user u where u.email = t1.host_email) as host_profile_image from dining_table t1 where host_email = ?`,
  babsangSelectedList: `select t1.*, (select u.profile_image from user u where u.email = t1.host_email) as host_profile_image from dining_table t1 inner join dining_table_spoons t2 on t1.id = t2.dining_table_id where t2.spoon_email = ? and t2.selected_yn = 'Y'`,
  babsangBookmarkedList: `select t1.*, t2.active_yn, (select u.profile_image from user u where u.email = t1.host_email) as host_profile_image from dining_table t1 inner join user_favorites t2 on t1.id = t2.dining_table_id where t2.user_email = ?`,
